...
import {
  ...
  ORDER_CONFIRM_PAY_REQUEST,
  ORDER_CONFIRM_PAY_FAIL,
  ORDER_CONFIRM_PAY_SUCCESS,
  ORDER_PAY_REQUEST,
  ORDER_PAY_FAIL,
  ORDER_PAY_SUCCESS,
...
export const confirmPay = (orderId) => async (dispatch, getState) => {
  dispatch({ type: ORDER_CONFIRM_PAY_REQUEST, payload: orderId });
  const {
    userSignin: { userInfo },
  } = getState();
  try {
    const { data } = Axios.put(
      `/api/orders/${orderId}/confirmPay`,
      {},
      {
        headers: { Authorization: `Bearer ${userInfo.token}` },
      }
    );
    dispatch({ type: ORDER_CONFIRM_PAY_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: ORDER_CONFIRM_PAY_FAIL, payload: message });
  }
};
