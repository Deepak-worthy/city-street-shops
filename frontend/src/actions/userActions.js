...
export const listSellers = ({pageNumber = '',city=''}) => async (dispatch) => {
  dispatch({ type: USER_SELLERS_LIST_REQUEST });
  try {
    const { data } = await Axios.get(`/api/users/sellers-List?pageNumber=${pageNumber}&city=${city}`);
...
export const listTopSellers = ({city=''}) => async (dispatch) => {
  dispatch({ type: USER_TOPSELLERS_LIST_REQUEST });
  try {
    const { data } = await Axios.get(`/api/users/top-sellers?city=${city}`);
